...
def lambda_handler(event, context):
    ...
    try: 
        ...
        send_sns_message("GrapheneSnakeOS Spot Instance FAILED", "There was a problem finding cheapest region for spot instance {}: {}".format(INSTANCE_TYPE, e))
        raise
    ...
    if float(cheapest_price) > float(SKIP_PRICE):
        ...
        send_sns_message("GrapheneSnakeOS Spot Instance SKIPPED", message)
        return message
...
- [ bash, -c, "sudo -u ubuntu bash /home/ubuntu/build.sh {1} {2} {3} {4}" ]
    ...
    try:
        ...
        if ENCRYPTED_KEYS == "true":
            ...
            send_sns_message("GrapheneSnakeOS Spot Instance CONFIGURATION ERROR", message)
            return message
        else:
    ...
    try:
        ...
        send_sns_message("GrapheneSnakeOS Spot Instance FAILED", "There was a problem requesting a spot instance {}: {}".format(INSTANCE_TYPE, e))
        raise

    subject = "GrapheneSnakeOS Spot Instance SUCCESS"
