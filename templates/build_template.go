...
aosp_repo_modifications() {
  ...
  if ! grep -q "GrapheneSnakeOS" .repo/manifest.xml; then
    ...
    awk -i inplace \
      ...
      print "  <project path=\"packages/apps/F-DroidPrivilegedExtension\" name=\"privileged-extension\" remote=\"fdroid\" revision=\"refs/tags/" FDROID_PRIV_EXT_VERSION "\" />";}' .repo/manifest.xml
