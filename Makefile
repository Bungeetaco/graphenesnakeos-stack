...
build:
	go build -race -ldflags "-X github.com/Bungeetaco/graphenesnakeos-stack/cli.version=$(VERSION)" -v -o "$(TARGET)" .

build-all:
	mkdir -v -p $(CURDIR)/artifacts/$(VERSION)
	gox -verbose -ldflags "-X github.com/Bungeetaco/graphenesnakeos-stack/cli.version=$(VERSION)" \
	    -os "$(OS)" -arch "$(ARCH)" \
...
zip: all
	...
	pushd artifacts/$(VERSION)/darwin && zip -r ../../../artifacts/zips/graphenesnakeos-stack-osx-${VERSION}.zip $(TARGET) && popd
	pushd artifacts/$(VERSION)/windows && zip -r ../../../artifacts/zips/graphenesnakeos-stack-windows-${VERSION}.zip $(TARGET).exe && popd
	pushd artifacts/$(VERSION)/linux && zip -r ../../../artifacts/zips/graphenesnakeos-stack-linux-${VERSION}.zip $(TARGET) && popd